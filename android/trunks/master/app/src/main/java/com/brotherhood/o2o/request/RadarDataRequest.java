...
import com.brotherhood.o2o.model.explore.RadarItem;
...
public class RadarDataRequest extends BaseAppRequest<ResponseResult<RadarItem>> {
    ...
    public RadarDataRequest(String url, OnBaseResponseListener<ResponseResult<RadarItem>> baseResponseListener) {
        super(url, Method.GET, true, null, baseResponseListener);
    }
    /**
     * longitude: 经度
     * latitude: 纬度
    ...
    public static RadarDataRequest createRadarDataRequest(OnResponseListener<RadarItem> listener){
        ...
        //todo 参数的longitude对应定位数据的mLatitude  是否存在问题？
        String longitude = String.valueOf(LocationComponent.shareComponent().getCachedCurrentAddressOrNil().mLatitude);
        String latitude = String.valueOf(LocationComponent.shareComponent().getCachedCurrentAddressOrNil().mLongitude);
