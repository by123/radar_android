...
public class ImageConfig {
    ...
    private static final int MAX_DISK_CACHE_SIZE = 60 * ByteConstants.MB;//默认图磁盘缓存的最大值
    ...
    public static ImagePipelineConfig configureCaches(Context context) {
        ...
        DiskCacheConfig diskSmallCacheConfig = DiskCacheConfig.newBuilder()
                ...
                .setBaseDirectoryName(Constants.ImageSmallDir)//文件夹名
...
//            .setVersion(version)
        ...
        DiskCacheConfig diskCacheConfig = DiskCacheConfig.newBuilder()
                ...
                .setBaseDirectoryName(Constants.ImageDir)//文件夹名
//            .setCacheErrorLogger(cacheErrorLogger)//日志记录器用于日志错误的缓存。
//            .setCacheEventListener(cacheEventListener)//缓存事件侦听器。
//            .setDiskTrimmableRegistry(diskTrimmableRegistry)//类将包含一个注册表的缓存减少磁盘空间的环境。
                .setMaxCacheSize(MAX_DISK_CACHE_SIZE)//默认缓存的最大大小。
