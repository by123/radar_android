...
public class RadarContentUI extends RelativeLayout {
	...
	private static final String TAG = "RadarContentUI";
	...
	private void RadarViewMove(int action) {
		try {
			if (MainActivity.mActivity != null
					&& MainActivity.mActivity.mExploreFragment != null) {
				ExploreFragment radarFragment = (ExploreFragment) MainActivity.mActivity.mExploreFragment;
				if (radarFragment != null) {
					Message msg = radarFragment.mHandler.obtainMessage(action);
					msg.sendToTarget();
				}
			}
		} catch (Exception e) {
	...
	private void HideResultView() {
		if (MainActivity.mActivity != null
			...
			if (CacheRef.getInstance().getmUserId() > 0) {
				ExploreFragment radarFragment = (ExploreFragment) MainActivity.mActivity.mExploreFragment;
				Message msg = radarFragment.mHandler
	...
	private void ShowChangePosition() {
		if (MainActivity.mActivity != null
				&& MainActivity.mActivity.mExploreFragment != null) {
			if (CacheRef.getInstance().getmUserId() > 0) {
				ExploreFragment mExploreFragment = (ExploreFragment) MainActivity.mActivity.mExploreFragment;
				mExploreFragment.mHandler
