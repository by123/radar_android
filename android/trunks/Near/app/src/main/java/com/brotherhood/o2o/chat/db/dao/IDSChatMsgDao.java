 ...
 */
public class IDSChatMsgDao extends BaseDBDao {

    private static final String tag = "IDSChatMsgDao";
    private static final int TABLE_VERSION = 1;
    private static IDSChatMsgDao instance;

    private IDSChatMsgDao(Context cxt) {
        super(cxt);
    }

    public static IDSChatMsgDao getInstance(Context context) {
        if (instance == null) {
            instance = new IDSChatMsgDao(context);
        }
        return instance;
    }
...
//                bean.sendStatus = c.getInt(c.getColumnIndex(ColumnItem.SEND_STATE));
    ...
    public void deleteTable(long uid) {
        try {
            getWritableDB().execSQL("DROP TABLE " + getTableName(uid));
        } catch (SQLiteException e) {
            e.printStackTrace();
        }
    }
    ...
    private class ColumnItem {
        public static final String TABLE = "chat_table_";
